...
namespace io::network
{
	...
	tcp_client::tcp_client (std::string __interface, std::string __address, short __port) {
		/*
		init_socket();
	...
	std::error_code tcp_client::init_write_socket()
	{
	...
	int tcp_client::init_read_socket()
	{
		return 1;
	}
	...
	bool tcp_client::bind_to_interface(const std::string &__interface) {
            struct ifreq s_ifr;
	...
	int tcp_client::get_binary_ip(const std::string& __ipv4)
        {
            int ip;
        ...
        int tcp_client::get_inteface_index(const std::string& __interface)
        {
            struct ifreq s_ifr;
            strcpy(s_ifr.ifr_ifrn.ifrn_name, __interface.c_str());
	...
	std::error_code tcp_client::init_socket(_socket& __socket) {
		__socket.fd = ::socket(PF_PACKET, SOCK_RAW, htons(0x00));
		if (__socket.fd == -1) {
			return std::make_error_code(std::errc::bad_file_descriptor);
		}
		if (setsockopt(__socket.fd, SOL_PACKET, PACKET_VERSION, &(__socket.packet_version),
	...
	std::error_code tcp_client::enable_timestamp(const _socket& __socket) const 
	{
	    int req = SOF_TIMESTAMPING_TX_HARDWARE
            | SOF_TIMESTAMPING_RX_HARDWARE 
            | SOF_TIMESTAMPING_TX_SOFTWARE
            | SOF_TIMESTAMPING_RX_SOFTWARE 
            | SOF_TIMESTAMPING_RAW_HARDWARE;
            if (setsockopt(__socket.fd, SOL_PACKET, SO_TIMESTAMPING, (void *) &req, sizeof(req))) {
                return std::make_error_code(std::errc::invalid_argument);
            }
            return std::error_code();
	}

	bool tcp_client::setup_ring_buffer(_socket& __socket, int __type) {
		...
		if (setsockopt(__socket.fd, SOL_PACKET, __type, (void *) &(__socket.buffer.req), sizeof(__socket.buffer.req))) {
			return false;
		}
		return true;
	}

	void tcp_client::v3_fill(_socket& __socket) {
		}

	void tcp_client::create_mmap(_socket& __socket) {
		__socket.buffer.map = static_cast<uint8_t *>(mmap(NULL, __socket.buffer.req.tp_block_size * __socket.buffer.req.tp_block_nr,
					PROT_READ | PROT_WRITE, MAP_SHARED | MAP_LOCKED, __socket.fd, 0));
		if (__socket.buffer.map == MAP_FAILED) {
			return;
		}
		__socket.buffer.rd = static_cast<iovec *>(malloc(__socket.buffer.req.tp_block_nr * sizeof(*__socket.buffer.rd)));
		for (auto i = 0; i < __socket.buffer.req.tp_block_nr; ++i) {
			__socket.buffer.rd[i].iov_base = __socket.buffer.map + (i * __socket.buffer.req.tp_block_size);
			__socket.buffer.rd[i].iov_len = __socket.buffer.req.tp_block_size;
		}
	}

	void tcp_client::config_poll(_socket& __socket) {
	...
	void tcp_client::update_statistics(_socket& __socket) {
		int err;
	...
	void tcp_client::walk_block(block_desc *pbd) {
	...
	void tcp_client::filter_and_dispatch_packet(tpacket3_hdr *ppd) {
...
*/
        ...
        void tcp_client::flush_block(block_desc *pbd) {
            pbd->h1.block_status = TP_STATUS_KERNEL;
        }

        void tcp_client::teardown_socket() {
